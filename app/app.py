...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # pic.bind("<Button-1>", self.callback)



    def callback(self, event):
        if self.bool:
            self.loading = Loading(parent=self, controller=self.controller)
            self.loading.place(x=150, y=150, anchor=tk.CENTER)
            self.bool = False
        else:
...
class Loading(tk.Frame):
    ...
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller=controller
        tonka = tk.Label(self, text="Loading...")
        tonka.pack()
        load = Image.open("loading.gif")
        ...
        for i in range(50):
            ...
            m = i + 1
            if (m < 42):
                filename = "donk/loading_00" + str(51-m) + "_Layer-" + str(m) + ".png"
            else:
                filename = "donk/loading_000" + str(51-m) + "_Layer-" + str(m) + ".png"
            print(filename)
            load = Image.open(filename)
            print(load.mode)
            load = load.resize((int(load.width/2), int(load.height/2)), Image.ANTIALIAS)
            test.append(ImageTk.PhotoImage(load))
        frames = [tk.PhotoImage(file='loading.gif', format='gif -index %i' % (i)) for i in range(51)]
        # frames[0].resize((40, 40), Image.ANTIALIAS)
        def update(ind):
            # print(ind)
            frame = test[ind]
            if ind < 49:
                ...
                ind = 0
            canvas.delete("all")
            # canvas.create_image(100, 100, image=frame)
            label.configure(image=frame)

            self.after(40, update, ind)

        label = tk.Label(self, bg="purple")
        canvas = tk.Canvas(self, width=0, height=0)
        # canvas.pack()
        # label.pack()
        # self.controller.wm_attributes("-transparentcolor", 'grey')
